...
Interfejs providera oddziela modele domenowe od prywatnego JSON: operacje live search, alternative dates i Explore parsują prywatny kontrakt wewnątrz `SkyscannerWebProvider`. Explore zwraca wyłącznie znormalizowane `ExploreSnapshot` / `ExploreResult`, a Autosuggest dla filtrów destynacji i wybranych krajów — `ResolvedExplorePlace` z osobną publiczną tożsamością. W tej ścieżce warstwa service nie interpretuje `everywhereDestination`, bucketów, quote'ów ani surowych pól destynacji z Autosuggest; providerowe ID destynacji pozostaje wyłącznie wewnętrznym kluczem payloadu Radar Explore. Istniejący resolver originów przekazuje surowy rekord Autosuggest, z którego service allowlistuje publiczną etykietę IATA/nazwę. Jedynym providerem jest `skyscanner_web`. Fast path używa klienta HTTP bez cookies; pomocniczy browser służy wyłącznie do ręcznego unlock.
