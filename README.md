...
CLI jest zaimplementowane w `src/flights_tracker/`; komendy `places resolve`, `search`, `alternative-dates`, `doctor` i `browser unlock` są dostępne po `uv sync`. `doctor` sprawdza DNS/TLS/HTTP i kontrakt Autosuggest, ale celowo nie tworzy kosztownego wyszukiwania Radar.
